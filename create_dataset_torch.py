...
class RhoDataset(Dataset):
  def __init__(self, dataset_dir, eval_dists, train = True):
    ...
    self.file_list = list()
    self.sample_count = 0
    for molecule in listdir(dataset_dir):
      if not isdir(join(dataset_dir, molecule)): continue
      for bond in listdir(join(dataset_dir, molecule)):
  ...
  def __getitem__(self, idx):
    ...
    return np.stack([density, grad_x, grad_y, grad_z], axis = 0), potential
