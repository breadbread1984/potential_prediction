...
class MLPMixer(nn.Module):
  def __init__(self, **kwargs):
    ...
    self.seq_len = kwargs.get('seq_len', 11**3+31)
    ...
    self.layernorm1 = nn.LayerNorm((self.seq_len, 1))
    ...
    for i in range(self.num_blocks):
      layers.update({
        'layernorm1_%d' % i: nn.LayerNorm((self.hidden_dim, self.seq_len)),
        'linear1_%d' % i: nn.Linear(self.seq_len, self.tokens_mlp_dim),
        'gelu1_%d' % i: nn.GELU(),
        'linear2_%d' % i: nn.Linear(self.tokens_mlp_dim, self.seq_len),
        'layernorm2_%d' % i: nn.LayerNorm((self.seq_len, self.hidden_dim)),
    ...
    self.layernorm2 = nn.LayerNorm((self.seq_len,self.hidden_dim))
...
if __name__ == "__main__":
  ...
  inputs = torch.randn(2, 1, 11**3+31)
